...
router.post(
  "/editProduct/:id",
  upload.array("newProductImage", 4),
...
router.get(
  "/inactive-coupons",
  adminAuth.isLogin,
  couponController.inactiveCouponsGET
)
...
router.post(
  ...
  bannerContoller.bannerDetailsAdd
)

router.get("/bannerlistUnlist/:id", bannerContoller.listUnlistBanner)

router.get("/editBanner/:id", bannerContoller.editBannerLoad)

router.post("/admin/editBanner/:id", bannerContoller.editBanner);

router.get("/salesProduct", orderController.productsTableOfSalesLoad)
